...
public final class CodelabBeanNaming {
  ...
  /**
   * Standard nomenclature: <module>.<InterfaceSimpleName> e.g. "inventory.OrderCreatedPublisher"
   *
   * <p>Namespaced by module to avoid collisions if two modules independently define an interface
   * with the same simple name (plausible — "CreatedPublisher" style naming tends to converge across
   * teams).
   */
  ...
  public static BeanNameGenerator moduleScoped(CodelabModule module) {
    return (definition, registry) -> {
      return module.name()
          + "."
          + AnnotationBeanNameGenerator.INSTANCE.generateBeanName(definition, registry);
    };
  }
}
