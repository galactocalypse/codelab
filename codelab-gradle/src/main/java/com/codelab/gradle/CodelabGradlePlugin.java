...
public class CodelabGradlePlugin implements Plugin<Project> {
  ...
  public void apply(Project project) {
    CodelabBuildExtension extension =
        project.getExtensions().create("codelab", CodelabBuildExtension.class);
    ...
    extension.getLicenseName().convention("The Apache Software License, Version 2.0");

    extension.getLicenseUrl().convention("https://www.apache.org/licenses/LICENSE-2.0.txt");

    extension.getScmUrl().convention("https://github.com/codelab");
    ...
    WrapperValidator.validate(project, extension.getGradleVersion().get());
  ...
  private void configurePom(
      Project project, MavenPublication publication, CodelabBuildExtension extension) {
    publication.getPom().getName().set(project.getName());

    publication
        .getPom()
        .getDescription()
        .set(project.getProviders().provider(project::getDescription));

    publication.getPom().getUrl().set(extension.getScmUrl());
    ...
    publication.getPom().scm(scm -> scm.getUrl().set(extension.getScmUrl()));
  }

  private void validatePublishingInformation(Project project, CodelabBuildExtension extension) {
    String group = project.getGroup().toString();
    String requiredPrefix = extension.getRequiredGroupPrefix().get();

    if ("undefined".equals(group) || !group.startsWith(requiredPrefix)) {
    ...
    if ("unspecified".equals(version)) {
      throw new IllegalStateException("Published projects must define an explicit version.");
    }

    String description = project.getDescription();

    if (description == null || description.isBlank()) {
      throw new IllegalStateException("Published projects must define a description.");
    }
  }

  private void configureJavaToolchain(Project project, CodelabBuildExtension extension) {
    project
        .getExtensions()
        .configure(
  ...
  private void checkGradleVersion(CodelabBuildExtension extension) {
    String actualVersion = GradleVersion.current().getVersion();

    String requiredVersion = extension.getGradleVersion().get();
  ...
  private void checkJavaVersion(CodelabBuildExtension extension) {
    int actualVersion = Runtime.version().feature();

    int requiredVersion = extension.getJavaVersion().get();
